...
void plotMeanBin() {
    ...
    if (plot != raw) data10->Scale(1/data10->Integral());
    ...
    if (plot != raw) data25->Scale(1/data25->Integral());
    ...
    if (plot != raw) data40->Scale(1/data40->Integral());
    ...
    // --------- Compute Integral Left and Right from Mean ------------------------------

    Double_t left, right;

    cout << endl;

    left = data10->Integral(1, data10->GetXaxis()->FindBin(mean[0]));
    right = data10->Integral(data10->GetXaxis()->FindBin(mean[0])+1,
                             data10->GetXaxis()->GetNbins());

    cout << " left: " << left << " | right: " << right
         << " | left + right: " << left + right << " | whole: " << data10->Integral() << endl;

    left = data25->Integral(1, data25->GetXaxis()->FindBin(mean[1]));
    right = data25->Integral(data25->GetXaxis()->FindBin(mean[1])+1,
                             data25->GetXaxis()->GetNbins());

    cout << " left: " << left << " | right: " << right
         << " | left + right: " << left + right << " | whole: " << data25->Integral() << endl;

    left = data40->Integral(1, data40->GetXaxis()->FindBin(mean[2]));
    right = data40->Integral(data40->GetXaxis()->FindBin(mean[2])+1,
                             data40->GetXaxis()->GetNbins());

    cout << " left: " << left << " | right: " << right
         << " | left + right: " << left + right << " | whole: " << data40->Integral() << endl;
    ...
    TGraph *gaussian10, *gaussian25, *gaussian40;

    gaussian->SetParameters(variance[0], mean[0]);

    gaussian10 = new TGraph(gaussian);
    gaussian10->SetLineColor(kBlack);
    gaussian10->SetLineWidth(3);
    gaussian10->SetLineStyle(1);

    cout << gaussian->Integral(0, 10) << endl;
    ...
    // ------------ Plotting ------------------------------------------------------------
        ...
        if (plot == gauss){
            ...
            hist1->Add(gaussian);//gaussian10
            hist1->Add(lInfo);

            output1 = makeCanvas(hist1, 0, "NoTime LogX LogY Thick Square", 0, 0);//

            data10->GetXaxis()->SetTitleOffset(1.5);
            output1->SetBottomMargin(0.11);

            output1->Update();
            output1->SaveAs("Momente/BinsGauss10.png");
            output1->SaveAs("Momente/BinsGauss10.root");
            ...
            output2 = makeCanvas(hist2, 0, "NoTime LogX LogY Thick", 0, 0);

            data25->GetXaxis()->SetTitleOffset(1.5);
            output2->SetBottomMargin(0.11);

            output2->Update();
            output2->SaveAs("Momente/BinsGauss25.png");
            output2->SaveAs("Momente/BinsGauss25.root");
            ...
            output3->SaveAs("Momente/BinsGauss40.png");
            output3->SaveAs("Momente/BinsGauss40.root");
            ...
            switch(plot){
                ...
                case raw:   output->SaveAs("Momente/Bins.png");
                            output->SaveAs("Momente/Bins.root");
                            break;

                case fit:   output->SaveAs("Momente/BinsFit.png");
                            output->SaveAs("Momente/BinsFit.root");
                            break;
